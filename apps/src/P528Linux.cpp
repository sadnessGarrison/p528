...
int main(int argc, char** argv) {
    ...
    case MODE_VERSION:
        printf("*******************************************************\n");
        printf("Institute for Telecommunications Sciences - Boulder, CO\n");
        printf("\tP.528 Driver Version: %i.%i\n", drvrVerMajor, drvrVerMinor);
        printf("\tP.528 Shared Object Version: %i.%i\n", dllVerMajor, dllVerMinor);
        printf("Time: %s", buf);
        printf("*******************************************************\n");
        break;
    default:
...
int CallP528_TABLE(DrvrParams* params) {
    ...
    if (!fp) {
        printf("Error opening output file (%s): %s. Exiting.\n", params->out_file, strerror(errno));
        return DRVRERR__VALIDATION_OUT_FILE;
    }

    fprintf(fp, "%fMHz / Lb(%f) dB\n", params->f__mhz, params->p);
    fprintf(fp, ",h2(m),1000,1000,1000,1000,1000,10000,10000,10000,10000,10000,10000,20000,20000,20000,20000,20000,20000,20000\n");
    fprintf(fp, ",h1(m),1.5,15,30,60,1000,1.5,15,30,60,1000,10000,1.5,15,30,60,1000,10000,20000\n");
    fprintf(fp, "D (km),FSL\n");
    ...
    for (int d__km = 0; d__km < CURVE_POINTS; d__km++) {
        ...
        fprintf(fp, ",%.3f", result.A_fs__db);
        fprintf(fp, ",%.3f", result.A__db);
...
int CallP528_POINT(DrvrParams* params) {
    ...
    if (params->mode == MODE_POINT && strlen(params->out_file) == 0)
    {
        ...
        if (rtn != SUCCESS && rtn != SUCCESS_WITH_WARNINGS) {
            printf("P.528 Returned Error Code: %i\n", rtn);
        }
        else {
            printf("Free Space Loss (dB): %.3f\n", result.A_fs__db);
            printf("Basic Transmission Loss (dB): %.3f\n", result.A__db);
            printf("Warning Flags: 0x%x\n", result.warnings);
        }
    }
    else {
        // Print results to file
        ...
        if (rtn != SUCCESS && rtn != SUCCESS_WITH_WARNINGS) {
            ...
            fprintf(fp, "Results\n");
            fprintf(fp, "Free Space Loss (dB),%.3f\n", result.A_fs__db);
            fprintf(fp, "Basic Transmission Loss (dB),%.3f\n", result.A__db);
            fprintf(fp, "DLL Return Code,%i\n", rtn);
            fprintf(fp, "Warning Flags,0x%x\n", result.warnings);
        }
